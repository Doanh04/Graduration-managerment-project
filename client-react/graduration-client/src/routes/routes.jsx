...
const columns = {
  ...
  templates: [{ key: 'templateId', label: 'Mã' }, { key: 'templateName', label: 'Tên biểu mẫu', primary: true, secondary: 'description' }, { key: 'filePath', label: 'Tệp', render: (value) => value ? <a className="file-link" href={value} target="_blank" rel="noreferrer" title={value}>{value}</a> : '—' }, { key: 'createAt', label: 'Ngày tạo', format: 'date' }],
...
const crud = {
  ...
  templates: { idKey: 'templateId', create: API_ENDPOINTS.templates.create, previewField: 'filePath', update: API_ENDPOINTS.templates.update, remove: API_ENDPOINTS.templates.remove, fields: [text('templateName', 'Tên biểu mẫu', true), text('description', 'Mô tả', false, { type: 'textarea' }), text('filePath', 'Đường dẫn tệp', true, { type: 'url', help: 'Sử dụng liên kết đã được chia sẻ công khai để mọi người có thể truy cập. Chỉ cấp quyền xem, không cấp quyền chỉnh sửa tệp.' })] },
};
...
export const Routers = [
  ...
  { path: '/access-denied', element: <AccessDeniedPage /> },
  {
    ...
    children: [
      ...
      { path: 'topics', element: <ModulePage title="Quản lý đề tài" description="Thêm, cập nhật, tra cứu và quản lý các đề tài đồ án tốt nghiệp." actionLabel="Tạo đề tài" endpoint={API_ENDPOINTS.topics.list} resourceParams={{ excludeStudentProposals: true }} columns={columns.topics} crud={{ ...crud.topics, detail: API_ENDPOINTS.topics.detail, detailView: 'topic' }} importEndpoint={API_ENDPOINTS.topics.import} serverSearch searchFields={['title', 'technology', 'teamName', 'status']} searchPlaceholder="Tìm theo tên đề tài, công nghệ, nhóm hoặc trạng thái..." /> },
